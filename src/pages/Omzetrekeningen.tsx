        ...
        {preview && (
          <div style={{ marginTop: 14, overflowX: 'auto', borderTop: '1px solid var(--border)', paddingTop: 12 }}>
            <table style={{ fontSize: 12, width: '100%' }}>
              <thead>
                <tr>
                  {['Datum','Rekening','Omschrijving','Debet','Credit','V.V.','Deb. nr','Deb. naam','Artikel omschr.','Artikel','Aantal'].map(h => (
              ...
              <tbody>
                ...
                {preview.length > 10 && (
                  <tr><td colSpan={11} style={{ padding: '6px 8px', color: 'var(--muted)', fontStyle: 'italic' }}>… en nog {preview.length - 10} regels</td></tr>
                )}
              </tbody>
            </table>
          </div>
        ...
        <div className="table-wrap">
          <table>
            ...
            <tbody>
              ...
              {!loading && filtered.length === 0 && <tr><td colSpan={11} className="empty">Geen regels gevonden</td></tr>}
              {filtered.map(r => (
                <tr key={r.id}>
